...
void atenderCliente(int * cliente) {
		...
		broker->id = BROKER;
		broker->idUnico = 100;
		t_handshake * idProcesoConectado = iniciarHandshake(*cliente, broker);
		...
		case GAMEBOY:;
			t_paquete* mensajeRecibido = recibir_mensaje(*cliente);
...
void lanzarHiloEscucha(int id, int socket){
	t_args_socket_escucha* argumentos = crear_args_socket_escucha(id, socket);
	pthread_t hilo;
	pthread_create(&hilo, NULL, (void*) escucharSocketMensajesACachear, (void*) argumentos);
	pthread_detach(hilo);
}

void lanzarHiloEscuchaACK(int id, int socket, op_code cola){
	t_args_socket_ACK* argumentos = crear_args_socket_escucha_ACK(id, socket, cola);
	pthread_t hilo;
	pthread_create(&hilo, NULL, (void*) escucharSocketACK, (void*) argumentos);
	pthread_detach(hilo);
}

void escucharSocketMensajesACachear(void* socketArgs){

	t_args_socket_escucha* args = socketArgs;
...
t_args_socket_escucha* crear_args_socket_escucha(int id, int socket){
	t_args_socket_escucha* argsARetornar = malloc(sizeof(t_args_socket_escucha));
	argsARetornar->id = id;
	argsARetornar->socket = socket;
	return argsARetornar;
}

t_args_socket_ACK* crear_args_socket_escucha_ACK(int id, int socket, op_code code){
	t_args_socket_ACK* argsARetornar = malloc(sizeof(t_args_socket_ACK));
	argsARetornar->id = id;
	argsARetornar->socket = socket;
	argsARetornar->cola = code;
	return argsARetornar;
}
